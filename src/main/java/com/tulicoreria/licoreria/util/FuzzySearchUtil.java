...
public final class FuzzySearchUtil {
    ...
    public static double scoreProducto(ProductoResponseDTO p, String query) {
       ...
       // Presentación (750 ML, 3 L, 20 CAJA, 12 PAQUETE)
if (p.getCantidadPresentacion() != null) {

    best = Math.max(
            best,
            scoreField(String.valueOf(p.getCantidadPresentacion()), query) * 0.55
    );

    if (p.getUnidadPresentacion() != null) {

        String presentacion =
                p.getCantidadPresentacion() + " " + p.getUnidadPresentacion();

        best = Math.max(
                best,
                scoreField(presentacion, query) * 0.60
        );

        best = Math.max(
                best,
                scoreField(p.getUnidadPresentacion(), query) * 0.50
        );
    }
}
        return best;
